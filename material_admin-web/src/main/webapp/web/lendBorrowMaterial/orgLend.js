var totalPage;
var pageSize = 13;
getOrgLendMaterial(1);

function getOrgLendMaterial(pageNum){
    $.ajax({
        type:"GET",
        url:"/lendBorrow/getOrgLendMaterial",
        data:"pageNum="+pageNum+"&pageSize="+pageSize,
        success:function(data){
            if(data.code === 200){
                showOrgLendMaterial(data.data);
...
function showPage(totalPage){
    ...
    for(var i=page;i>=1;i--){
        div.prepend('<li><a href="#" onclick="getOrgLendMaterial('+i+')">'+i+'</a></li>');
    }
}

function showOrgLendMaterial(list){
    $(".tr-data").hide();
